...
#[test]
fn test_log_config_display_all() {
    let log_dir = "test_logs_display_all";
    let log_file = "test_logs_display_all/test_display.log";
    if Path::new(log_dir).exists() { let _ = fs::remove_dir_all(log_dir); }
    let mut config = LogConfig::new(Some("test_display".to_string()), Some(log_file.to_string()), Some("debug".to_string()), Some("1M".to_string()), Some(2));
    ...
    let handle = thread::Builder::new().name("test-thread".to_string()).spawn(|| { tracing::info!("Message from test-thread"); }).unwrap();
    handle.join().unwrap();
    thread::sleep(Duration::from_millis(200));
    let content = fs::read_to_string(log_file).expect("Failed to read log file");
    println!("Log content: {}", content);
    assert!(content.contains("INFO"), "Display level failed");
    assert!(content.contains("log_config_display_all"), "Display target failed");
