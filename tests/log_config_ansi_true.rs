...
#[test]
fn test_log_config_ansi_true() {
    let log_dir = "test_logs_ansi_true";
    let log_file = "test_logs_ansi_true/test_ansi.log";
    if Path::new(log_dir).exists() { let _ = fs::remove_dir_all(log_dir); }
    let mut config = LogConfig::new(Some("test_ansi".to_string()), Some(log_file.to_string()), Some("info".to_string()), Some("1M".to_string()), Some(2));
