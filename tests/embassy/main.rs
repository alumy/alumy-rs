#![cfg(feature = "embassy")]
//! Skeleton integration tests for the Embassy platform layer.
//!
//! `embassy` currently only forwards the shared `core` helpers (see
//! AGENTS.md), so these tests check that the re-export wiring in
//! `src/embassy` stays intact rather than asserting platform-specific
//! behavior.
//!
//! As real platform APIs land in `alumy::embassy`, extend this file
